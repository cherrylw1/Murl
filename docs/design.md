...
|---|---|---|
| Signature / dot | **DotGothic16** (or **Departure Mono**) | Agent IDs, counters, costs, timers, status words, big numerals, the Glyph readouts. Restraint — never body. |
...
```
display-dot   28 / 32   DotGothic16     (hero numerals, run cost, big status)
...
micro-dot     10 / 12   DotGothic16      (cell labels, tiny counters)
```
...
- Agent ID in `micro-dot` (DotGothic16): `A1`, `A2`…
